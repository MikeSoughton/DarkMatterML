...
#print len(xedges)
xedges =np.linspace(100.0,1000.0, num=n_ybins) #changed this from 2000 to 2600 as there are still events up there and they are porbably the more important ones for distinguihsing signals
#print len(yedges)

BG_ptj,BG_etaj,BG_phij,BG_signal=np.loadtxt("smdelphes.csv", unpack=True,skiprows=1)
ALP_ptj,ALP_etaj,ALP_phij,ALP_signal=np.loadtxt("axiondelphes.csv", unpack=True,skiprows=1)
EFT_ptj,EFT_etaj,EFT_phij,EFT_signal=np.loadtxt("spin1meddelphes.csv", unpack=True,skiprows=1)
SUSY1_ptj,SUSY1_etaj,SUSY1_phij,SUSY1_signal=np.loadtxt("susy1delphes.csv", unpack=True,skiprows=1)
SUSY2_ptj,SUSY2_etaj,SUSY2_phij,SUSY2_signal=np.loadtxt("susy2delphes.csv", unpack=True,skiprows=1)
SUSY3_ptj,SUSY3_etaj,SUSY3_phij,SUSY3_signal=np.loadtxt("susy3delphes.csv", unpack=True,skiprows=1)
...
batched_BG_ptj=split(BG_ptj,N_images)
batched_BG_etaj=split(BG_etaj,N_images)

# Produce BG BGjet density arrays for training data with large number of events
for i in range(1,N_images+1):
      H_BG, xedges, yedges = np.histogram2d(batched_BG_ptj[i-1],batched_BG_etaj[i-1],bins=(xedges, yedges))
      H2 = H_BG
...
batched_ALP_ptj=split(ALP_ptj,N_images)
batched_ALP_etaj=split(ALP_etaj,N_images)

# Produce ALP ALPjet density arrays for training data with large number of events
for i in range(1,N_images+1):
      H_ALP, xedges, yedges = np.histogram2d(batched_ALP_ptj[i-1],batched_ALP_etaj[i-1],bins=(xedges, yedges))
      H2 = H_ALP
...
batched_EFT_ptj=split(EFT_ptj,N_images)
batched_EFT_etaj=split(EFT_etaj,N_images)

# Produce EFT density arrays for training data with large number of events
for i in range(1,N_images+1):
      H_EFT, xedges, yedges = np.histogram2d(batched_EFT_ptj[i-1],batched_EFT_etaj[i-1],bins=(xedges, yedges))
...
batched_SUSY1_ptj=split(SUSY1_ptj,N_images)
batched_SUSY1_etaj=split(SUSY1_etaj,N_images)

# Produce SUSY1 density arrays for training data with large number of events
for i in range(1,N_images+1):
      H_SUSY1, xedges, yedges = np.histogram2d(batched_SUSY1_ptj[i-1],batched_SUSY1_etaj[i-1],bins=(xedges, yedges))
...
batched_SUSY2_ptj=split(SUSY2_ptj,N_images)
batched_SUSY2_etaj=split(SUSY2_etaj,N_images)

# Produce SUSY2 density arrays for training data with large number of events
for i in range(1,N_images+1):
      H_SUSY2, xedges, yedges = np.histogram2d(batched_SUSY2_ptj[i-1],batched_SUSY2_etaj[i-1],bins=(xedges, yedges))
...
batched_SUSY3_ptj=split(SUSY3_ptj,N_images)
batched_SUSY3_etaj=split(SUSY3_etaj,N_images)

# Produce SUSY3 density arrays for training data with large number of events
for i in range(1,N_images+1):
      H_SUSY3, xedges, yedges = np.histogram2d(batched_SUSY3_ptj[i-1],batched_SUSY3_etaj[i-1],bins=(xedges, yedges))
...
plt.figure()
plt.plot(ALP_SUSY1bg_rate, ALP_SUSY1sig_rate, lw=2, color='red', linestyle='solid', label='SUSY 1 vs ALP, AUC = %0.2f)'%(ALP_SUSY1roc_auc))
plt.plot(EFT_SUSY1bg_rate, EFT_SUSY1sig_rate, lw=2, color='red', linestyle='dashed', label='SUSY 1 vs EFT, AUC = %0.2f)'%(EFT_SUSY1roc_auc))
plt.plot(ALP_SUSY2bg_rate, ALP_SUSY2sig_rate, lw=2, color='blue', linestyle='solid', label='SUSY 2 vs ALP, AUC = %0.2f)'%(ALP_SUSY2roc_auc))
plt.plot(EFT_SUSY2bg_rate, EFT_SUSY2sig_rate, lw=2, color='blue', linestyle='dashed', label='SUSY 2 vs EFT, AUC = %0.2f)'%(EFT_SUSY2roc_auc))
plt.plot(ALP_SUSY3bg_rate, ALP_SUSY3sig_rate, lw=2, color='green', linestyle='solid', label='SUSY 3 vs ALP, AUC = %0.2f)'%(ALP_SUSY3roc_auc))
plt.plot(EFT_SUSY3bg_rate, EFT_SUSY3sig_rate, lw=2, color='green', linestyle='dashed', label='SUSY 3 vs EFT, AUC = %0.2f)'%(EFT_SUSY3roc_auc))
plt.xlabel(r'$\epsilon_{s_{2}}$')
plt.ylabel(r'$\epsilon_{s_{1}}$')
plt.title(r'DNN')
legend1=plt.legend(loc='lower right',prop={'size':10})
plt.savefig('2D_DNN_SUSY_vs_sig_delphes_ROC.png')
...
plt.title(r'CNN')
legend1=plt.legend(loc='lower right',prop={'size':10})
plt.savefig('2D_CNN_SUSY_vs_sig_delphes_ROC.png')
