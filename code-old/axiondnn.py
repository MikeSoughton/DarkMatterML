...
#load our data files
signal=pd.read_csv("../analysis/monoj.csv",sep='\s+',engine='python')
bkgd=pd.read_csv("../analysis/spin1med.csv",sep='\s+',engine='python')
...
dnnConfigs={
    'dnn_ZH_0L_cHW_0d03_batch_1024':{'epochs':100,'batch_size':500,'dropOut':0.2,'l2Regularization':None,'hiddenLayers':[10.0,64,64,64],
...
for varSetName,varSet in chosenVars.items():
    ...
    for name,config in dnnConfigs.items():
        dnn = Dnn(mlData,'testPlots/mlPlots/binary_crossentropy/'+varSetName+'/'+name)
