...
N = 200
Nsmall = 4000 # here small refers to number of events, not number of batches
...
for i in range(1,Nsmall+1):
      H_small_mono, xedges, yedges = np.histogram2d(small_batched_mono_ptj[i-1],small_batched_mono_etaj[i-1],bins=(xedges, yedges))
...
for i in range(1,Nsmall+1):
      H_small_DM, xedges, yedges = np.histogram2d(small_batched_DM_ptj[i-1],small_batched_DM_etaj[i-1],bins=(xedges, yedges))
...
small_DM_signal=np.zeros((Nsmall,1))
...
print('Test accuracy:', score[1]) #now mae
...
probs = fcmodel.predict(x_test)
fpr, tpr, thresholds = roc_curve(y_test[:,1], probs[:,1])
sig1_rate = tpr
sig2_rate = 1 - fpr
roc_auc = auc(sig1_rate, sig2_rate)

small_probs = fcmodel.predict(small_x_test)
small_fpr, small_tpr, small_thresholds = roc_curve(small_y_test[:,1], small_probs[:,1])
small_sig1_rate = small_tpr
small_sig2_rate = 1 - small_fpr
small_roc_auc = auc(small_sig1_rate, small_sig2_rate)

plt.figure()
plt.plot(sig1_rate, sig2_rate, lw=2, label='ROC(area = %0.2f)'%(roc_auc))
plt.plot(small_sig1_rate, small_sig2_rate, lw=2, label='SMALL_ROC(area = %0.2f)'%(small_roc_auc))
legend1=plt.legend(loc='upper left',prop={'size':16})

plt.figure()
plt.plot(sig1_rate, 1.0/sig2_rate, lw=2, label='ROC(area = %0.2f)'%(roc_auc))
plt.plot(small_sig1_rate, 1.0/small_sig2_rate, lw=2, label='SMALL_ROC(area = %0.2f)'%(small_roc_auc))
legend1=plt.legend(loc='upper left',prop={'size':16})
...
def histplot_mae(history):
    ...
    min_mean_absolute_error = hist['val_mean_absolute_error'].min()
    ax2.hlines(min_mean_absolute_error, 0, len(hist), linestyle='dotted',
               label='min(test mean absolute error) = {:.3f}'.format(min_mean_absolute_error))
    ax2.legend(loc='upper right', fontsize='large')
    #plab.savefig('new1.pdf', bbox_inches=0,dpi=100)

#plt.figure(1)
#histplot(history1)
#histplot_mae(history1)
...
small_x_train = small_x_train.reshape(-1, n_xbins-1, n_ybins-1, 1)
small_x_test = small_x_test.reshape(-1, n_xbins-1, n_ybins-1, 1)
...
history3 = cnnmodel.fit(x_train, y_train,
                       batch_size=batch_size,
                       epochs=1*epochs,
                       verbose=2,
                       validation_data=(x_test, y_test))
...
score = cnnmodel.evaluate(small_x_test, small_y_test, verbose=0)
print('Test loss:', score[0])
print('Test accuracy:', score[1]) #now mae
