...
yedges =np.linspace(-4.0, 4.0, num=n_xbins)
#print len(xedges)
xedges =np.linspace(130.0,2000.0, num=n_ybins)
#print len(yedges)



BG_ptj,BG_etaj,BG_phij,BG_signal=np.loadtxt("smnp1.csv", unpack=True,skiprows=1)
ALP_ptj,ALP_etaj,ALP_phij,ALP_signal=np.loadtxt("alpnp1.csv", unpack=True,skiprows=1)
EFT_ptj,EFT_etaj,EFT_phij,EFT_signal=np.loadtxt("spin1med.csv", unpack=True,skiprows=1)
SUSY1_ptj,SUSY1_etaj,SUSY1_phij,SUSY1_signal=np.loadtxt("susycor1.csv", unpack=True,skiprows=1)
SUSY2_ptj,SUSY2_etaj,SUSY2_phij,SUSY2_signal=np.loadtxt("susycor2.csv", unpack=True,skiprows=1)
SUSY3_ptj,SUSY3_etaj,SUSY3_phij,SUSY3_signal=np.loadtxt("susycor3.csv", unpack=True,skiprows=1)
...
plt.plot(ALPbg_rate, ALPsig_rate, lw=2,color='red', label='ALPs, AUC = %0.2f'%ALProc_auc)
plt.plot(EFTbg_rate, EFTsig_rate, lw=2,color='green', label='EFT, AUC = %0.2F'%(EFTroc_auc))
plt.plot(SUSY1bg_rate, SUSY1sig_rate, lw=2, color='orange', label='SUSY1, AUC = %0.2f'%(SUSY1roc_auc))
plt.plot(SUSY2bg_rate, SUSY2sig_rate, lw=2, color='blue', label='SUSY2, AUC = %0.2f'%(SUSY2roc_auc))
...
legend2=plt.legend([r"Detector Level"],loc=5,prop={'size':10},handlelength=0, handletextpad=0,frameon=False)
ax.add_artist(legend1)
plt.savefig('2D_DNN_sig_vs_bg_delphes_ROC.png')
...
plt.savefig('2D_CNN_sig_vs_bg_delphes_ROC.png')
