...
def rocCurve(y_preds,y_test=None,output=None,append=''):
    ...
    for name,y_pred in y_preds.iteritems():
        ...
        plt.plot(tpr, fpr, lw=2, label=name+' (AUC = %0.2f)'%(roc_auc))
...
#    plt.plot([0, 1], [0, 1], '--', color=(0.6, 0.6, 0.6), label='Luck')
    ...
    plt.xlim([-0.05, 1.05])
    plt.ylim([-0.05, 1.05])
    plt.xlabel(r'$\epsilon_{s_1}$',fontsize=18)
    plt.ylabel(r'$\epsilon_{s_2}$',fontsize=18)

#    plt.title('Receiver operating characteristic')
    legend1=plt.legend(loc="upper left")
    legend2=plt.legend([r"NN"],loc=4,prop={'size':16},handlelength=0, handletextpad=0,frameon=False)
